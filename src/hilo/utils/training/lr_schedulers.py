...
def initialize_lr_scheduler(lr_scheduler_cfg, optimizer, num_iterations_per_epoch):
    if lr_scheduler_cfg is None or lr_scheduler_cfg.get("type", "none").lower() == "none":
        return None
    ...
    if lr_scheduler_type in ["onecycle", "one_cycle", "onecyclelr", "one_cycle_lr"]:
        ...
        return OneCycle(optimizer, max_lr, warmup_epochs, max_epochs, num_iterations_per_epoch)
    else:
        raise NotImplementedError(f"LR scheduler type '{lr_scheduler_type}' is not implemented yet.")
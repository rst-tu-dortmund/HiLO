...
def save_topk_checkpoints(
    ...
    if topk_checkpoints is None:
        ...
        for metric in checkpoint_cfg["metrics"]:
            ...
            print(f"Saved first checkpoint for {metrics[metric]:.4f} {metric} at epoch {epoch} to {topk_checkpoints[metric]['paths'][-1]}")
    else:
        for metric in checkpoint_cfg["metrics"]:
            ...
            if len(topk_checkpoints[metric]["epochs"]) < checkpoint_cfg["topk"]:
                ...
                topk_checkpoints[metric]["metrics"] = np.append(topk_checkpoints[metric]["metrics"], current_metric)
                ...
                print(f"Saved new checkpoint for {current_metric:.4f} {metric} at epoch {epoch} to {path}")

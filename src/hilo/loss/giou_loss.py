...
class NonRotatedBevGIoULoss(torch.nn.Module):
    ...
    def forward(self, boxes1, boxes2):
        ...
        """
        giou = non_rotated_bev_giou(boxes1, boxes2).diagonal()  # (B, N) giou values
...
def non_rotated_bev_giou(boxes1, boxes2):
    conv_boxes1 = box_convert(boxes1[..., :4], in_fmt="cxcywh", out_fmt="xyxy")

    conv_boxes2 = box_convert(boxes2[..., :4], in_fmt="cxcywh", out_fmt="xyxy")
...
class AxisAlignedBevGIoULoss(torch.nn.Module):
    ...
    def forward(self, boxes1, boxes2):
        ...
        giou = axis_aligned_bev_giou(boxes1, boxes2).diagonal()  # (B, N) giou values
...
def compute_generalized_iou(aa_boxes1, aa_boxes2):
    areas1 = (aa_boxes1[..., 2] - aa_boxes1[..., 0]) * (
        aa_boxes1[..., 3] - aa_boxes1[..., 1]
    )
    areas2 = (aa_boxes2[..., 2] - aa_boxes2[..., 0]) * (
        aa_boxes2[..., 3] - aa_boxes2[..., 1]
    )

    max_mins = torch.max(
        aa_boxes1[..., None, :2], aa_boxes2[..., None, :, :2]
    )  # [B,N,M,2]
    min_maxs = torch.min(
        aa_boxes1[..., None, 2:4], aa_boxes2[..., None, :, 2:4]
    )  # [B,N,M,2]
...
def compute_rotated_corners(boxes1, corners):
    ...
    boxes1_rot = torch.stack(
        [
            torch.stack(
                [torch.cos(boxes1[..., 4]), -torch.sin(boxes1[..., 4])], dim=-1
            ),

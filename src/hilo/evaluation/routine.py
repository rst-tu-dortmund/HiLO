...
def evaluate_model(model, val_loader, epoch, metrics_cfg, use_wandb=False, visualize_cfg=None):
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(val_loader, desc=f"Evaluating Epoch {epoch}")):
        ...
        if use_wandb:
            log_dict = {
                f"avg/eval/{k}": v for k, v in epoch_metrics.items()
            }
...
def should_evaluate(cfg, epoch, epochs):
    return (epoch + 1) % cfg.get("interval", 1) == 0 or epoch + 1 >= cfg.get("eval_after_epoch", epochs)
        ...
        ret_res = {
            ...
            "theshold_wise_F1": {
                threshold: (
                    ...
                    if (2 * res[threshold]["true_positives"]
                        + res[threshold]["false_positives"]
                        + res[threshold]["false_negatives"]
                    ) > 0
                    else 0.0
            ...
            "threshold_wise_classification_accuracy": {
                threshold: (
                    ...
                    / (res[threshold]["true_classified"] + res[threshold]["false_classified"])
                    if (res[threshold]["true_classified"] + res[threshold]["false_classified"]) > 0
                    else 0.0
                )
                for threshold in self.config["THRESHOLDS"]
            },
    ...
    def eval_batch_threshold(self, data, threshold):
        ...
        for iou_matrix, det_classes, gt_classes in zip(
            ...
            assignments = np.stack(
                                   linear_sum_assignment(full_iou_matrix, maximize=True) # returns row_ind, col_ind
                                   , axis=1
                                   )
        ...
        res = {
            ...
            "mIoU": sum_assigned_ious / sum_true_positives if sum_true_positives > 0 else 0.0,
    ...
    def combine_batches(self, all_res):
        ...
        for res in all_res.values():
            for threshold in self.config["THRESHOLDS"]:
                ...
                combined_stats[threshold]["sum_assigned_ious"] += stats["sum_assigned_ious"]
                combined_stats[threshold]["true_classified"] += stats["true_classified"]
                combined_stats[threshold]["false_classified"] += stats["false_classified"]
